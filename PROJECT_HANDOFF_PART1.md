...
| Frontend runtime verification | **0 / 17 open items** | FE-RV-3/4/6/17 require live infra; **FE-RV-7…16 are BLOCKED, additionally, by an undelivered backend `/api/v1` dependency, not infra alone** (only `health.router` is mounted; auth is unwired; see the FE-RV-7 Blocker Report); see `webplatform/frontend/PRODUCTION_READINESS_RUNBOOK.md` — the frontend's analogue of `RUNTIME_VERIFICATION_REGISTRY.md` |
